...
<HTML>
<HEAD>
<TITLE>Comparación de tres versiones de la Biblia</TITLE>
...
if Len(Request.Form("ID")) then
	...
	for i = 1 to 3
		...
		Select Case i
			Case 1
				...
				bibliaName = "Versión Reina-Valera"
			Case 2
		...
		if not objRec.EOF then
			While not objRec.EOF
				...
				if i = 1 then
					if j = 1 then
						bibliaValera = bibliaValera & "<TR><TD><H4>" & bibliaName & "</H4></TD></TR>"
						bibliaValera = bibliaValera & "<TR><TD><B>" & libro_nombre & " " & new_cap & ":" & Request("vers_num") & "</B></TD></TR>"
					end if
					bibliaValera = bibliaValera & "<TR><TD CLASS=""small""><SUP>" & vers_num & "</SUP> " & vers_txt & "</TD></TR>" & VbCrLf
					j = j + 1
				elseif i = 2 then
					if j = 1 then
						bibliaOtra = bibliaOtra & "<TR><TD><H4>" & bibliaName & "</H4></TD><TR>"
						bibliaOtra = bibliaOtra & "<TR><TD><B>" & libro_nombre & " " & new_cap & ":" & Request("vers_num") & "</B></TD></TR>"
					end if
					bibliaOtra = bibliaOtra & "<TR><TD CLASS=""small""><SUP>" & vers_num & "</SUP> " & vers_txt & "</TD></TR>" & VbCrLf
					j = j + 1		
				else
					if j = 1 then
						bibliaJerusalen = bibliaJerusalen & "<TR><TD><H4>" & bibliaName & "</H4></TD></TR>"
						bibliaJerusalen = bibliaJerusalen & "<TR><TD><B>" & libro_nombre & " " & new_cap & ":" & Request("vers_num") & "</B></TD></TR>"
					end if
	...
	Response.Write "<TABLE BORDER=""0"" CELLPADDING=""20"" CELLSPACING=""0"" WIDTH=""100%""><TR><TD BGCOLOR=""pink"" VALIGN=""TOP"" WIDTH=""30%"">" & VbCrLf
	
	Response.Write "<TABLE BORDER=""0"" CELLPADDING=""3"" CELLSPACING=""0"" WIDTH=""100%"">" & VbCrLf
	Response.Write bibliaValera
	Response.Write "</TABLE>" & VbCrLf
	
	Response.Write "</TD><TD WIDTH=""5%"">&nbsp;&nbsp;&nbsp;</TD><TD BGCOLOR=""lightgreen"" VALIGN=""TOP"" WIDTH=""30%"">"

	Response.Write "<TABLE BORDER=""0"" CELLPADDING=""3"" CELLSPACING=""0"" WIDTH=""100%"">" & VbCrLf
	Response.Write bibliaOtra
	Response.Write "</TABLE>" & VbCrLf

	Response.Write "</TD><TD WIDTH=""5%"">&nbsp;&nbsp;&nbsp;</TD><TD BGCOLOR=""lightblue"" VALIGN=""TOP"" WIDTH=""30%"">"
